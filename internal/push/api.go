...
func (a *API) list(w http.ResponseWriter, r *http.Request) {
	...
	for _, s := range subs {
		out = append(out, viewOf(s, latest.Get(r.Context(), s.Source)))
	}
...
func (a *API) get(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, viewOf(sub, latest))
}
...
func viewOf(s store.PushSubscription, latestSeq int64) subView {
	return subView{
		ID: s.ID, Source: s.Source, TargetURL: s.TargetURL, Name: s.Name,
		Cursor:              s.Cursor,
		QueueDepth:          store.QueueDepth(latestSeq, s.Cursor),
