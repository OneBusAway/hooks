...
// requireAdmin enforces admin scope via the cookie token. On failure, GET
// requests redirect to /inspector/login; non-GET return 403.
func (in *Inspector) requireAdmin(w http.ResponseWriter, r *http.Request) (store.Token, bool) {
	...
	tok, err := in.Tokens.LookupByPlaintext(r.Context(), c.Value)
	if err != nil {
		clearCookie(w)
		if r.Method == http.MethodGet {
			http.Redirect(w, r, "/inspector/login", http.StatusFound)
	...
	_ = in.Tokens.TouchLastUsed(r.Context(), tok.ID, time.Now().UTC())
	return tok, true
}
...
func (in *Inspector) loginPOST(w http.ResponseWriter, r *http.Request) {
	...
	tok, err := in.Tokens.LookupByPlaintext(r.Context(), plaintext)
...
func (in *Inspector) index(w http.ResponseWriter, r *http.Request) {
	...
	storedSources, _ := in.Events.Sources(r.Context())
	configured := append([]string{}, in.Sources...)
	...
	const limit = 50
	rows, err := in.recentEvents(r.Context(), selected, limit)
	if err != nil {
...
func (in *Inspector) recentEvents(ctx context.Context, sourceFilter string, limit int) ([]indexRow, error) {
	sources := []string{}
	if sourceFilter != "" {
		sources = []string{sourceFilter}
	} else {
		stored, err := in.Events.Sources(ctx)
		if err != nil {
			return nil, err
		}
		sources = stored
...
func (in *Inspector) renderSubs(ctx context.Context, subs []store.PushSubscription) ([]subRow, error) {
	...
	for _, s := range subs {
		latest, err := in.Events.LatestSequence(ctx, s.Source)
		if err != nil {
			return nil, err
		}
...
func (in *Inspector) pushCreate(w http.ResponseWriter, r *http.Request) {
	...
	cursor, _ := in.Events.LatestSequence(r.Context(), source)

	sub := store.PushSubscription{
		ID: newID(), Source: source, TargetURL: target, Name: name,
	...
	subs, _ := in.Subs.List(r.Context(), true)
	rendered, _ := in.renderSubs(r.Context(), subs)
	in.render(w, "push", map[string]any{
		"Title":     "Push",
		"Subs":      rendered,
		"Plaintext": plaintext,
...
func newID() string {
	var raw [16]byte
	_, _ = cryptoRandRead(raw[:])
	return hexEncode(raw[:])
}

// silence unused import after refactor
var _ = json.Indent
